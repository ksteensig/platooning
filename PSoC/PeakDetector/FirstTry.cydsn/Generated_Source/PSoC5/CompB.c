...
uint8 CompB_initVar = 0u;

/* Internal functions definitoin */
static void CompB_trimAdjust(uint8 nibble) ;

/* static CompB_backupStruct  CompB_backup; */
#if (CY_PSOC5A)
    static CompB_LOWPOWER_BACKUP_STRUCT  CompB_lowPowerBackup;
#endif /* CY_PSOC5A */

/* variable to decide whether or not to restore the control register in 
   Enable() API for PSoC5A only */
#if (CY_PSOC5A)
    static uint8 CompB_restoreReg = 0u;
#endif /* CY_PSOC5A */


/*******************************************************************************
* Function Name: CompB_Init
********************************************************************************
*
* Summary:
...
void CompB_Init(void) 
{
    ...
    CompB_SetSpeed(CompB_DEFAULT_SPEED);

    /* Set default Hysteresis */
    #if ( CompB_DEFAULT_HYSTERESIS == 0u )
        CompB_CR |= CompB_HYST_OFF;
    #else
        CompB_CR &= (uint8)(~CompB_HYST_OFF);
    #endif /* CompB_DEFAULT_HYSTERESIS == 0u */
    /* Power down override feature is not supported for PSoC5A. */
    #if (CY_PSOC3 || CY_PSOC5LP)
        /* Set default Power Down Override */
        #if ( CompB_DEFAULT_PWRDWN_OVRD == 0u )
            CompB_CR &= (uint8)(~CompB_PWRDWN_OVRD);
        #else 
            CompB_CR |= CompB_PWRDWN_OVRD;
        #endif /* CompB_DEFAULT_PWRDWN_OVRD == 0u */
    #endif /* CY_PSOC3 || CY_PSOC5LP */
    
    /* Set mux always on logic */
    CompB_CR |= CompB_MX_AO;

    /* Set default sync */
    CompB_CLK &= (uint8)(~CompB_SYNCCLK_MASK);
    #if ( CompB_DEFAULT_BYPASS_SYNC == 0u )
        CompB_CLK |= CompB_SYNC_CLK_EN;
    #else
        CompB_CLK |= CompB_BYPASS_SYNC;
    #endif /* CompB_DEFAULT_BYPASS_SYNC == 0u */
}


/*******************************************************************************
* Function Name: CompB_Enable
********************************************************************************
*
* Summary:
...
{
    CompB_PWRMGR |= CompB_ACT_PWR_EN;
    CompB_STBY_PWRMGR |= CompB_STBY_PWR_EN;
     
     /* This is to restore the value of register CR which is saved 
    in prior to the modification in stop() API */
    #if (CY_PSOC5A)
        if(CompB_restoreReg == 1u)
        {
            CompB_CR = CompB_lowPowerBackup.compCRReg;
...
*
* Global variables:
*  CompB_initVar: Is modified when this function is called for the 
...
void CompB_Stop(void) 
{
    ...
    CompB_PWRMGR &= (uint8)(~CompB_ACT_PWR_EN);
    CompB_STBY_PWRMGR &= (uint8)(~CompB_STBY_PWR_EN);
        ...
        CompB_lowPowerBackup.compCRReg = CompB_CR;
        CompB_CR = CompB_COMP_REG_CLR;
    #endif /* CY_PSOC5A */
}


/*******************************************************************************
* Function Name: CompB_SetSpeed
********************************************************************************
*
* Summary:
...
void CompB_SetSpeed(uint8 speed) 
{
    ...
    CompB_CR = (CompB_CR & (uint8)(~CompB_PWR_MODE_MASK)) |
                           (speed & CompB_PWR_MODE_MASK);

    /* Set trim value for high speed comparator */
    if(speed == CompB_HIGHSPEED)
    {
        /* PSoC5A */
        #if (CY_PSOC5A)
            CompB_TR = CompB_HS_TRIM_TR0;
        #endif /* CY_PSOC5A */
        
        /* PSoC3, PSoC5LP or later */
        #if (CY_PSOC3 || CY_PSOC5LP) 
            CompB_TR0 = CompB_HS_TRIM_TR0;
            CompB_TR1 = CompB_HS_TRIM_TR1;
        #endif /* CY_PSOC3 || CY_PSOC5LP */
    }
    else
    {
    /* PSoC5A */
        #if (CY_PSOC5A)
            CompB_TR = CompB_LS_TRIM_TR0;
        #endif /* CY_PSOC5A */
        
        /* PSoC3, PSoC5LP or later */
        #if (CY_PSOC3 || CY_PSOC5LP) 
            CompB_TR0 = CompB_LS_TRIM_TR0;
            CompB_TR1 = CompB_LS_TRIM_TR1;
        #endif /* CY_PSOC3 || CY_PSOC5LP */
    }
...
/*******************************************************************************
* Function Name: CompB_GetCompare
********************************************************************************
*
* Summary:
...
{
    return( CompB_WRK & CompB_CMP_OUT_MASK);
}


/*******************************************************************************
* Function Name: CompB_trimAdjust
********************************************************************************
*
* Summary:
...
static void CompB_trimAdjust(uint8 nibble) 
{
    ...
    cmpState = CompB_WRK & CompB_CMP_OUT_MASK;
    ...
    {    
        ...
        if ( cmpState != 0u )
        {
            /* PSoC5A */
            #if (CY_PSOC5A)
                CompB_TR |= CompB_CMP_TRIM1_DIR;
            #endif /* CY_PSOC5A */
            
            /* PSoC3, PSoC5LP or later */
            #if (CY_PSOC3 || CY_PSOC5LP)
                CompB_TR0 |= CompB_CMP_TR0_DIR;
            #endif /* CY_PSOC3 || CY_PSOC5LP */
        }
    }
    else
    {
        ...
        if ( cmpState == 0u )
        {
            /* PSoC5A */
            #if (CY_PSOC5A)
                CompB_TR |= CompB_CMP_TRIM2_DIR; 
            #endif /* CY_PSOC5A */
            
            /* PSoC3, PSoC5LP or later */
            #if (CY_PSOC3 || CY_PSOC5LP)
                CompB_TR1 |= CompB_CMP_TR1_DIR;
            #endif /* CY_PSOC3 || CY_PSOC5LP */
        }
    }
    ...
    for ( trimCnt = 0u; trimCnt < trimCntMax; trimCnt++ )
	{
        ...
        if ((CompB_WRK & CompB_CMP_OUT_MASK) != cmpState)
        {
...
uint16 CompB_ZeroCal(void) 
{
    ...
    CompB_SW0 = 0x00u;
    CompB_SW2 = 0x00u;
    CompB_SW3 = tmpSW3 & (uint8)(~CompB_CMP_SW3_INPCTL_MASK);
    ...
    */
    tmpCR = CompB_CR;
    CompB_CR |= (CompB_CAL_ON | CompB_HYST_OFF);
    
    /* Write default low values to trim register - no offset adjust */
    /* PSoC5A */
    #if (CY_PSOC5A)
        CompB_TR = CompB_DEFAULT_CMP_TRIM;
    #endif /* CY_PSOC5A */
    
    /* PSoC3, PSoC5LP or later */
    #if (CY_PSOC3 || CY_PSOC5LP)
        CompB_TR0 = CompB_DEFAULT_CMP_TRIM;
        CompB_TR1 = CompB_DEFAULT_CMP_TRIM;
    #endif /* CY_PSOC3 || CY_PSOC5LP */
	
	/* Two phase trim - slow modes, one phase trim - for fast */ 
    if ( (CompB_CR & CompB_PWR_MODE_MASK) == CompB_PWR_MODE_FAST)
    {
        CompB_trimAdjust(0u);
    }
    ...
    /* PSoC5A */
    #if (CY_PSOC5A)
        return (uint16) CompB_TR;
    #endif /* CY_PSOC5A */
    
    /* PSoC3, PSoC5LP or later */
    #if (CY_PSOC3 || CY_PSOC5LP)
        return ((uint16)((uint16)CompB_TR1 << 8u) | (CompB_TR0));        
    #endif /* CY_PSOC3 || CY_PSOC5LP */
}


/*******************************************************************************
* Function Name: CompB_LoadTrim
********************************************************************************
*
* Summary:
...
void CompB_LoadTrim(uint16 trimVal) 
{
    ...
    /*******************************************************************************
    * Function Name: CompB_PwrDwnOverrideEnable
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    ...
    /*******************************************************************************
    * Function Name: CompB_PwrDwnOverrideDisable
    ********************************************************************************
    *
    * Summary:
