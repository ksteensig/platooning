...
uint8 Comp_PD_B_initVar = 0u;

/* Internal functions definitoin */
static void Comp_PD_B_trimAdjust(uint8 nibble) ;

/* static Comp_PD_B_backupStruct  Comp_PD_B_backup; */
#if (CY_PSOC5A)
    static Comp_PD_B_LOWPOWER_BACKUP_STRUCT  Comp_PD_B_lowPowerBackup;
#endif /* CY_PSOC5A */

/* variable to decide whether or not to restore the control register in 
   Enable() API for PSoC5A only */
#if (CY_PSOC5A)
    static uint8 Comp_PD_B_restoreReg = 0u;
#endif /* CY_PSOC5A */


/*******************************************************************************
* Function Name: Comp_PD_B_Init
********************************************************************************
*
* Summary:
...
void Comp_PD_B_Init(void) 
{
    ...
    Comp_PD_B_SetSpeed(Comp_PD_B_DEFAULT_SPEED);

    /* Set default Hysteresis */
    #if ( Comp_PD_B_DEFAULT_HYSTERESIS == 0u )
        Comp_PD_B_CR |= Comp_PD_B_HYST_OFF;
    #else
        Comp_PD_B_CR &= (uint8)(~Comp_PD_B_HYST_OFF);
    #endif /* Comp_PD_B_DEFAULT_HYSTERESIS == 0u */
    /* Power down override feature is not supported for PSoC5A. */
    #if (CY_PSOC3 || CY_PSOC5LP)
        /* Set default Power Down Override */
        #if ( Comp_PD_B_DEFAULT_PWRDWN_OVRD == 0u )
            Comp_PD_B_CR &= (uint8)(~Comp_PD_B_PWRDWN_OVRD);
        #else 
            Comp_PD_B_CR |= Comp_PD_B_PWRDWN_OVRD;
        #endif /* Comp_PD_B_DEFAULT_PWRDWN_OVRD == 0u */
    #endif /* CY_PSOC3 || CY_PSOC5LP */
    
    /* Set mux always on logic */
    Comp_PD_B_CR |= Comp_PD_B_MX_AO;

    /* Set default sync */
    Comp_PD_B_CLK &= (uint8)(~Comp_PD_B_SYNCCLK_MASK);
    #if ( Comp_PD_B_DEFAULT_BYPASS_SYNC == 0u )
        Comp_PD_B_CLK |= Comp_PD_B_SYNC_CLK_EN;
    #else
        Comp_PD_B_CLK |= Comp_PD_B_BYPASS_SYNC;
    #endif /* Comp_PD_B_DEFAULT_BYPASS_SYNC == 0u */
}


/*******************************************************************************
* Function Name: Comp_PD_B_Enable
********************************************************************************
*
* Summary:
...
{
    Comp_PD_B_PWRMGR |= Comp_PD_B_ACT_PWR_EN;
    Comp_PD_B_STBY_PWRMGR |= Comp_PD_B_STBY_PWR_EN;
     
     /* This is to restore the value of register CR which is saved 
    in prior to the modification in stop() API */
    #if (CY_PSOC5A)
        if(Comp_PD_B_restoreReg == 1u)
        {
            Comp_PD_B_CR = Comp_PD_B_lowPowerBackup.compCRReg;

            /* Clear the flag */
            Comp_PD_B_restoreReg = 0u;
        }
    #endif /* CY_PSOC5A */
}
...
*
* Global variables:
*  Comp_PD_B_initVar: Is modified when this function is called for the 
...
void Comp_PD_B_Start(void) 
{
    ...
    if ( Comp_PD_B_initVar == 0u )
    {
        Comp_PD_B_Init();
        
        Comp_PD_B_initVar = 1u;
    }   
...
void Comp_PD_B_Stop(void) 
{
    ...
    Comp_PD_B_PWRMGR &= (uint8)(~Comp_PD_B_ACT_PWR_EN);
    Comp_PD_B_STBY_PWRMGR &= (uint8)(~Comp_PD_B_STBY_PWR_EN);
    
    #if (CY_PSOC5A)
        /* Enable the variable */
        Comp_PD_B_restoreReg = 1u;

        /* Save the control register before clearing it */
        Comp_PD_B_lowPowerBackup.compCRReg = Comp_PD_B_CR;
        Comp_PD_B_CR = Comp_PD_B_COMP_REG_CLR;
    #endif /* CY_PSOC5A */
}


/*******************************************************************************
* Function Name: Comp_PD_B_SetSpeed
********************************************************************************
*
* Summary:
...
void Comp_PD_B_SetSpeed(uint8 speed) 
{
    ...
    Comp_PD_B_CR = (Comp_PD_B_CR & (uint8)(~Comp_PD_B_PWR_MODE_MASK)) |
                           (speed & Comp_PD_B_PWR_MODE_MASK);

    /* Set trim value for high speed comparator */
    if(speed == Comp_PD_B_HIGHSPEED)
    {
        /* PSoC5A */
        #if (CY_PSOC5A)
            Comp_PD_B_TR = Comp_PD_B_HS_TRIM_TR0;
        #endif /* CY_PSOC5A */
        
        /* PSoC3, PSoC5LP or later */
        #if (CY_PSOC3 || CY_PSOC5LP) 
            Comp_PD_B_TR0 = Comp_PD_B_HS_TRIM_TR0;
            Comp_PD_B_TR1 = Comp_PD_B_HS_TRIM_TR1;
        #endif /* CY_PSOC3 || CY_PSOC5LP */
    }
    else
    {
    /* PSoC5A */
        #if (CY_PSOC5A)
            Comp_PD_B_TR = Comp_PD_B_LS_TRIM_TR0;
        #endif /* CY_PSOC5A */
        
        /* PSoC3, PSoC5LP or later */
        #if (CY_PSOC3 || CY_PSOC5LP) 
            Comp_PD_B_TR0 = Comp_PD_B_LS_TRIM_TR0;
            Comp_PD_B_TR1 = Comp_PD_B_LS_TRIM_TR1;
        #endif /* CY_PSOC3 || CY_PSOC5LP */
    }
...
/*******************************************************************************
* Function Name: Comp_PD_B_GetCompare
********************************************************************************
*
* Summary:
...
{
    return( Comp_PD_B_WRK & Comp_PD_B_CMP_OUT_MASK);
}


/*******************************************************************************
* Function Name: Comp_PD_B_trimAdjust
********************************************************************************
*
* Summary:
...
static void Comp_PD_B_trimAdjust(uint8 nibble) 
{
    ...
    cmpState = Comp_PD_B_WRK & Comp_PD_B_CMP_OUT_MASK;
    ...
    {    
        ...
        if ( cmpState != 0u )
        {
            /* PSoC5A */
            #if (CY_PSOC5A)
                Comp_PD_B_TR |= Comp_PD_B_CMP_TRIM1_DIR;
            #endif /* CY_PSOC5A */
            
            /* PSoC3, PSoC5LP or later */
            #if (CY_PSOC3 || CY_PSOC5LP)
                Comp_PD_B_TR0 |= Comp_PD_B_CMP_TR0_DIR;
            #endif /* CY_PSOC3 || CY_PSOC5LP */
        }
    }
    else
    {
        ...
        if ( cmpState == 0u )
        {
            /* PSoC5A */
            #if (CY_PSOC5A)
                Comp_PD_B_TR |= Comp_PD_B_CMP_TRIM2_DIR; 
            #endif /* CY_PSOC5A */
            
            /* PSoC3, PSoC5LP or later */
            #if (CY_PSOC3 || CY_PSOC5LP)
                Comp_PD_B_TR1 |= Comp_PD_B_CMP_TR1_DIR;
            #endif /* CY_PSOC3 || CY_PSOC5LP */
        }
    }
    ...
    for ( trimCnt = 0u; trimCnt < trimCntMax; trimCnt++ )
	{
        ...
        if ((Comp_PD_B_WRK & Comp_PD_B_CMP_OUT_MASK) != cmpState)
        {
...
/*******************************************************************************
* Function Name: Comp_PD_B_ZeroCal
********************************************************************************
*
* Summary:
...
uint16 Comp_PD_B_ZeroCal(void) 
{
    ...
    tmpSW0 = Comp_PD_B_SW0;
    tmpSW2 = Comp_PD_B_SW2;
    tmpSW3 = Comp_PD_B_SW3;

     /* Clear routing for inP, retain routing for inN */
    Comp_PD_B_SW0 = 0x00u;
    Comp_PD_B_SW2 = 0x00u;
    Comp_PD_B_SW3 = tmpSW3 & (uint8)(~Comp_PD_B_CMP_SW3_INPCTL_MASK);
    ...
    */
    tmpCR = Comp_PD_B_CR;
    Comp_PD_B_CR |= (Comp_PD_B_CAL_ON | Comp_PD_B_HYST_OFF);
    
    /* Write default low values to trim register - no offset adjust */
    /* PSoC5A */
    #if (CY_PSOC5A)
        Comp_PD_B_TR = Comp_PD_B_DEFAULT_CMP_TRIM;
    #endif /* CY_PSOC5A */
    
    /* PSoC3, PSoC5LP or later */
    #if (CY_PSOC3 || CY_PSOC5LP)
        Comp_PD_B_TR0 = Comp_PD_B_DEFAULT_CMP_TRIM;
        Comp_PD_B_TR1 = Comp_PD_B_DEFAULT_CMP_TRIM;
    #endif /* CY_PSOC3 || CY_PSOC5LP */
	
	/* Two phase trim - slow modes, one phase trim - for fast */ 
    if ( (Comp_PD_B_CR & Comp_PD_B_PWR_MODE_MASK) == Comp_PD_B_PWR_MODE_FAST)
    {
        Comp_PD_B_trimAdjust(0u);
    }
    else /* default to trim for fast modes */
    {
        Comp_PD_B_trimAdjust(1u);
		Comp_PD_B_trimAdjust(0u);
    }
    ...
    Comp_PD_B_SW0 = tmpSW0;
    Comp_PD_B_SW2 = tmpSW2;
    Comp_PD_B_SW3 = tmpSW3;
    
    /* PSoC5A */
    #if (CY_PSOC5A)
        return (uint16) Comp_PD_B_TR;
    #endif /* CY_PSOC5A */
    
    /* PSoC3, PSoC5LP or later */
    #if (CY_PSOC3 || CY_PSOC5LP)
        return ((uint16)((uint16)Comp_PD_B_TR1 << 8u) | (Comp_PD_B_TR0));        
    #endif /* CY_PSOC3 || CY_PSOC5LP */
}


/*******************************************************************************
* Function Name: Comp_PD_B_LoadTrim
********************************************************************************
*
* Summary:
...
void Comp_PD_B_LoadTrim(uint16 trimVal) 
{
    ...
    /*******************************************************************************
    * Function Name: Comp_PD_B_PwrDwnOverrideEnable
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    ...
    /*******************************************************************************
    * Function Name: Comp_PD_B_PwrDwnOverrideDisable
    ********************************************************************************
    *
    * Summary:
