...
void ServoClock_Start(void) 
{
    ...
    ServoClock_CLKEN |= ServoClock_CLKEN_MASK;
	ServoClock_CLKSTBY |= ServoClock_CLKSTBY_MASK;
}
...
void ServoClock_Stop(void) 
{
    ...
    ServoClock_CLKEN &= (uint8)(~ServoClock_CLKEN_MASK);
	ServoClock_CLKSTBY &= (uint8)(~ServoClock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: ServoClock_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((ServoClock_CLKEN & ServoClock_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = ServoClock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = ServoClock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* ServoClock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(ServoClock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(ServoClock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: ServoClock_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: ServoClock_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = ServoClock_GetSourceRegister();
    uint16 oldDivider = ServoClock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = ServoClock_CLKEN & ServoClock_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((ServoClock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
{
    return CY_GET_REG16(ServoClock_DIV_PTR);
}


/*******************************************************************************
* Function Name: ServoClock_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    ServoClock_MOD_SRC |= modeBitMask & (uint8)ServoClock_MODE_MASK;
}


/*******************************************************************************
* Function Name: ServoClock_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    ServoClock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(ServoClock_MODE_MASK));
}


/*******************************************************************************
* Function Name: ServoClock_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return ServoClock_MOD_SRC & (uint8)(ServoClock_MODE_MASK);
}


/*******************************************************************************
* Function Name: ServoClock_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = ServoClock_GetDividerRegister();
    uint8 oldSrc = ServoClock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        ServoClock_MOD_SRC |= CYCLK_SSS;
        ServoClock_MOD_SRC =
            (ServoClock_MOD_SRC & (uint8)(~ServoClock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: ServoClock_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return ServoClock_MOD_SRC & ServoClock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: ServoClock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    ServoClock_PHASE = clkPhase & ServoClock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: ServoClock_GetPhase
********************************************************************************
*
* Summary:
...
{
    return ServoClock_PHASE & ServoClock_PHASE_MASK;
}
