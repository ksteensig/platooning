...
#include "CE.h"
...
#if defined(CE__PC)
    #define CE_SetP4PinDriveMode(shift, mode)  \
    do { \
        CE_PC =   (CE_PC & \
                                (uint32)(~(uint32)(CE_DRIVE_MODE_IND_MASK << \
                                (CE_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << \
                                (CE_DRIVE_MODE_BITS * (shift))); \
    } while (0)
#else
        ...
        #define CE_SetP4PinDriveMode(shift, mode)  \
        do { \
            CE_USBIO_CTRL_REG = (CE_USBIO_CTRL_REG & \
                                    (uint32)(~(uint32)(CE_DRIVE_MODE_IND_MASK << \
                                    (CE_DRIVE_MODE_BITS * (shift))))) | \
                                    (uint32)((uint32)(mode) << \
                                    (CE_DRIVE_MODE_BITS * (shift))); \
    ...
    *******************************************************************************/
    ...
    {
		CE_SetP4PinDriveMode(CE__0__SHIFT, mode);
    }
#endif
...
void CE_SetInterruptMode(uint16 position, uint16 mode)
{
    ...
    intrCfg =  CE_INTCFG & (uint32)(~(uint32)position);
    CE_INTCFG = intrCfg | ((uint32)position & (uint32)mode);
}
