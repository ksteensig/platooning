...
{
    uint8 staticBits = (speaker_DR & (uint8)(~speaker_MASK));
    speaker_DR = staticBits | ((uint8)(value << speaker_SHIFT) & speaker_MASK);
}
...
{
	CyPins_SetPinDriveMode(speaker_0, mode);
}
...
{
    return (speaker_PS & speaker_MASK) >> speaker_SHIFT;
}
...
{
    return (speaker_DR & speaker_MASK) >> speaker_SHIFT;
}


/* If interrupt is connected for this Pins component */ 
#if defined(speaker_INTSTAT) 
    ...
    *******************************************************************************/
    ...
    {
		if((position & speaker_0_INTR) != 0u) 
		{ 
			 speaker_0_INTTYPE_REG = (uint8)mode; 
		}
    }
    ...
    *******************************************************************************/
    ...
    {
        return (speaker_INTSTAT & speaker_MASK) >> speaker_SHIFT;
    }
