...
***************************************/
static void ADC_SAR_B_CalcGain(uint8 resolution);
...
volatile int16 ADC_SAR_B_offset;
volatile int16 ADC_SAR_B_countsPerVolt;     /* Obsolete Gain compensation */
volatile int32 ADC_SAR_B_countsPer10Volt;   /* Gain compensation */
volatile int16 ADC_SAR_B_shift;
...
*  ADC_SAR_B_Start() sets the initVar variable, calls the
*  ADC_SAR_B_Init() function, and then calls the
*  ADC_SAR_B_Enable() function.
*
* Parameters:
*  None.
*
...
void ADC_SAR_B_Init(void)
{
    ...
    #if(ADC_SAR_B_IRQ_REMOVE == 0u)
        /* Start and set interrupt vector */
        CyIntSetPriority(ADC_SAR_B_INTC_NUMBER, ADC_SAR_B_INTC_PRIOR_NUMBER);
        (void)CyIntSetVector(ADC_SAR_B_INTC_NUMBER, &ADC_SAR_B_ISR);
    #endif   /* End ADC_SAR_B_IRQ_REMOVE */
...
void ADC_SAR_B_Enable(void)
{
    ...
    ADC_SAR_B_STBY_PWRMGR_SAR_REG |= ADC_SAR_B_STBY_PWR_SAR_EN;

    /* This is only valid if there is an internal clock */
    #if(ADC_SAR_B_DEFAULT_INTERNAL_CLK)
        ADC_SAR_B_PWRMGR_CLK_REG |= ADC_SAR_B_ACT_PWR_CLK_EN;
        ADC_SAR_B_STBY_PWRMGR_CLK_REG |= ADC_SAR_B_STBY_PWR_CLK_EN;
    #endif /* End ADC_SAR_B_DEFAULT_INTERNAL_CLK */

    /* Enable VCM buffer and Enable Int Ref Amp */
    tmpReg = ADC_SAR_B_SAR_CSR3_REG;
    tmpReg |= ADC_SAR_B_SAR_EN_BUF_VCM_EN;
    /* PD_BUF_VREF is OFF in External reference or Vdda reference mode */
    #if((ADC_SAR_B_DEFAULT_REFERENCE == ADC_SAR_B__EXT_REF) || \
        (ADC_SAR_B_DEFAULT_RANGE == ADC_SAR_B__VNEG_VDDA_DIFF))
        tmpReg &= (uint8)~ADC_SAR_B_SAR_EN_BUF_VREF_EN;
    #else /* In INTREF or INTREF Bypassed this buffer is ON */
        tmpReg |= ADC_SAR_B_SAR_EN_BUF_VREF_EN;
    #endif /* ADC_SAR_B_DEFAULT_REFERENCE == ADC_SAR_B__EXT_REF */
    ADC_SAR_B_SAR_CSR3_REG = tmpReg;

    /* Set reference for ADC */
    #if(ADC_SAR_B_DEFAULT_RANGE == ADC_SAR_B__VNEG_VDDA_DIFF)
        #if(ADC_SAR_B_DEFAULT_REFERENCE == ADC_SAR_B__EXT_REF)
            ADC_SAR_B_SAR_CSR6_REG = ADC_SAR_B_INT_BYPASS_EXT_VREF; /* S2 */
        #else /* Internal Vdda reference or obsolete bypass mode */
            ADC_SAR_B_SAR_CSR6_REG = ADC_SAR_B_VDDA_VREF;           /* S7 */
        #endif /* ADC_SAR_B_DEFAULT_REFERENCE == ADC_SAR_B__EXT_REF */
    #else  /* Reference goes through internal buffer */
        #if(ADC_SAR_B_DEFAULT_REFERENCE == ADC_SAR_B__INT_REF_NOT_BYPASSED)
            ADC_SAR_B_SAR_CSR6_REG = ADC_SAR_B_INT_VREF;            /* S3 + S4 */
        #else /* INTREF Bypassed of External */
            ADC_SAR_B_SAR_CSR6_REG = ADC_SAR_B_INT_BYPASS_EXT_VREF; /* S2 */
        #endif /* ADC_SAR_B_DEFAULT_REFERENCE == ADC_SAR_B__INT_REF_NOT_BYPASSED */
    #endif /* VNEG_VDDA_DIFF */

    /* Low non-overlap delay for sampling clock signals (for 1MSPS) */
    #if(ADC_SAR_B_HIGH_POWER_PULSE == 0u) /* MinPulseWidth <= 50 ns */
        ADC_SAR_B_SAR_CSR5_REG &= (uint8)~ADC_SAR_B_SAR_DLY_INC;
    #else /* Set High non-overlap delay for sampling clock signals (for <500KSPS)*/
        ADC_SAR_B_SAR_CSR5_REG |= ADC_SAR_B_SAR_DLY_INC;
    #endif /* ADC_SAR_B_HIGH_POWER_PULSE == 0u */
    ...
    */
    #if(ADC_SAR_B_HIGH_POWER_PULSE == 0u)    /* MinPulseWidth <= 50 ns */
        ADC_SAR_B_SAR_CSR5_REG |= ADC_SAR_B_SAR_SEL_CSEL_DFT_CHAR;
    #else /* for <500ksps */
        ADC_SAR_B_SAR_CSR5_REG &= (uint8)~ADC_SAR_B_SAR_SEL_CSEL_DFT_CHAR;
    #endif /* ADC_SAR_B_HIGH_POWER_PULSE == 0u */

    /* Set default power and other configurations for control register 0 in multiple lines */
    ADC_SAR_B_SAR_CSR0_REG = (uint8)((uint8)ADC_SAR_B_DEFAULT_POWER << ADC_SAR_B_SAR_POWER_SHIFT)
    /* SAR_HIZ_CLEAR:   Should not be used for LP */
    #if ((CY_PSOC5LP) || (ADC_SAR_B_DEFAULT_REFERENCE != ADC_SAR_B__EXT_REF))
        | ADC_SAR_B_SAR_HIZ_CLEAR
    #endif /* SAR_HIZ_CLEAR */
    /*Set Convertion mode */
    #if(ADC_SAR_B_DEFAULT_CONV_MODE != ADC_SAR_B__FREE_RUNNING)      /* If triggered mode */
        | ADC_SAR_B_SAR_MX_SOF_UDB           /* source: UDB */
        | ADC_SAR_B_SAR_SOF_MODE_EDGE        /* Set edge-sensitive sof source */
    #endif /* ADC_SAR_B_DEFAULT_CONV_MODE */
    ; /* end of multiple line initialization */

    ADC_SAR_B_SAR_TR0_REG = ADC_SAR_B_SAR_CAP_TRIM_2;
...
void ADC_SAR_B_Stop(void)
{
    ...
    ADC_SAR_B_SAR_CSR0_REG &= (uint8)~ADC_SAR_B_SAR_SOF_START_CONV;
    /* Disable the SAR ADC block in Active Power Mode */
    ADC_SAR_B_PWRMGR_SAR_REG &= (uint8)~ADC_SAR_B_ACT_PWR_SAR_EN;
    /* Disable the SAR ADC in Standby Power Mode */
    ADC_SAR_B_STBY_PWRMGR_SAR_REG &= (uint8)~ADC_SAR_B_STBY_PWR_SAR_EN;

    /* This is only valid if there is an internal clock */
    #if(ADC_SAR_B_DEFAULT_INTERNAL_CLK)
        ADC_SAR_B_PWRMGR_CLK_REG &= (uint8)~ADC_SAR_B_ACT_PWR_CLK_EN;
        ADC_SAR_B_STBY_PWRMGR_CLK_REG &= (uint8)~ADC_SAR_B_STBY_PWR_CLK_EN;
    #endif /* End ADC_SAR_B_DEFAULT_INTERNAL_CLK */
...
void ADC_SAR_B_SetPower(uint8 power)
{
    ...
    /* Set Power parameter  */
    tmpReg = ADC_SAR_B_SAR_CSR0_REG & (uint8)~ADC_SAR_B_SAR_POWER_MASK;
    tmpReg |= (uint8)(power << ADC_SAR_B_SAR_POWER_SHIFT);
...
void ADC_SAR_B_SetResolution(uint8 resolution)
{
    ...
    switch (resolution)
    {
        case (uint8)ADC_SAR_B__BITS_12:
            tmpReg = ADC_SAR_B_SAR_RESOLUTION_12BIT | ADC_SAR_B_SAR_SAMPLE_WIDTH;
            break;
        case (uint8)ADC_SAR_B__BITS_10:
            tmpReg = ADC_SAR_B_SAR_RESOLUTION_10BIT | ADC_SAR_B_SAR_SAMPLE_WIDTH;
            break;
        case (uint8)ADC_SAR_B__BITS_8:
            tmpReg = ADC_SAR_B_SAR_RESOLUTION_8BIT | ADC_SAR_B_SAR_SAMPLE_WIDTH;
            break;
        default:
            tmpReg = ADC_SAR_B_SAR_RESOLUTION_12BIT | ADC_SAR_B_SAR_SAMPLE_WIDTH;
    ...
    *  ADC_SAR_B_StartConvert(). This function is not available when the
    ...
    *******************************************************************************/
    ...
    {
        #if(ADC_SAR_B_DEFAULT_CONV_MODE != ADC_SAR_B__FREE_RUNNING)  /* If software triggered mode */
            ADC_SAR_B_SAR_CSR0_REG &= (uint8)~ADC_SAR_B_SAR_MX_SOF_UDB;   /* source: SOF bit */
        #endif /* End ADC_SAR_B_DEFAULT_CONV_MODE */
...
*   ADC_SAR_B_RETURN_STATUS: Immediately returns the status. If the
...
*   ADC_SAR_B_WAIT_FOR_RESULT: Does not return a result until the ADC
...
uint8 ADC_SAR_B_IsEndConversion(uint8 retMode)
{
    ...
    if(status == ADC_SAR_B_SAR_EOF_1)
    {
        ...
        CY_GET_REG8(ADC_SAR_B_SAR_CSR1_PTR);
    }
...
*
* Global Variables:
*  ADC_SAR_B_shift - used to convert the ADC counts to the 2s
*  compliment form.
*
* Side Effects:
...
{
    return( (int8)ADC_SAR_B_SAR_WRK0_REG - (int8)ADC_SAR_B_shift);
}
...
int16 ADC_SAR_B_GetResult16( void )
{
    ...
    res = CY_GET_REG16(ADC_SAR_B_SAR_WRK_PTR);

    return( (int16)res - ADC_SAR_B_shift );
}
...
*
* Summary:
*  Sets the ADC offset, which is used by ADC_SAR_B_CountsTo_Volts(),
*  ADC_SAR_B_CountsTo_mVolts(), and ADC_SAR_B_CountsTo_uVolts()
...
*  ADC_SAR_B_countsPer10Volt variable initialized. This variable is used
...
{
    int32 counts;
    #if(!((ADC_SAR_B_DEFAULT_RANGE == ADC_SAR_B__VSS_TO_VREF) || \
         (ADC_SAR_B_DEFAULT_RANGE == ADC_SAR_B__VSSA_TO_VDDA) || \
         (ADC_SAR_B_DEFAULT_RANGE == ADC_SAR_B__VSSA_TO_VDAC)) )
        uint16 diff_zero;
    #endif /* End ADC_SAR_B_DEFAULT_RANGE */

    switch (resolution)
    {
        case (uint8)ADC_SAR_B__BITS_12:
            counts = (int32)ADC_SAR_B_SAR_WRK_MAX_12BIT;
            #if(!((ADC_SAR_B_DEFAULT_RANGE == ADC_SAR_B__VSS_TO_VREF) || \
                 (ADC_SAR_B_DEFAULT_RANGE == ADC_SAR_B__VSSA_TO_VDDA) || \
                 (ADC_SAR_B_DEFAULT_RANGE == ADC_SAR_B__VSSA_TO_VDAC)) )
                diff_zero = ADC_SAR_B_SAR_DIFF_SHIFT;
            #endif /* End ADC_SAR_B_DEFAULT_RANGE */
            break;
        case (uint8)ADC_SAR_B__BITS_10:
            counts = (int32)ADC_SAR_B_SAR_WRK_MAX_10BIT;
            #if(!((ADC_SAR_B_DEFAULT_RANGE == ADC_SAR_B__VSS_TO_VREF) || \
                 (ADC_SAR_B_DEFAULT_RANGE == ADC_SAR_B__VSSA_TO_VDDA) || \
                 (ADC_SAR_B_DEFAULT_RANGE == ADC_SAR_B__VSSA_TO_VDAC)) )
                diff_zero = ADC_SAR_B_SAR_DIFF_SHIFT >> 2u;
            #endif /* End ADC_SAR_B_DEFAULT_RANGE */
            break;
        case (uint8)ADC_SAR_B__BITS_8:
            counts = (int32)ADC_SAR_B_SAR_WRK_MAX_8BIT;
            #if(!((ADC_SAR_B_DEFAULT_RANGE == ADC_SAR_B__VSS_TO_VREF) || \
                 (ADC_SAR_B_DEFAULT_RANGE == ADC_SAR_B__VSSA_TO_VDDA) || \
                 (ADC_SAR_B_DEFAULT_RANGE == ADC_SAR_B__VSSA_TO_VDAC)) )
                diff_zero = ADC_SAR_B_SAR_DIFF_SHIFT >> 4u;
            #endif /* End ADC_SAR_B_DEFAULT_RANGE */
            break;
    ...
    ADC_SAR_B_countsPerVolt = 0; /* Clear obsolete variable */
    /* Calculate gain in counts per 10 volts with rounding */
    ADC_SAR_B_countsPer10Volt = (((counts * ADC_SAR_B_10MV_COUNTS) +
                        ADC_SAR_B_DEFAULT_REF_VOLTAGE_MV) / (ADC_SAR_B_DEFAULT_REF_VOLTAGE_MV * 2));
...
*
* Global Variables:
*  ADC_SAR_B_countsPer10Volt variable modified. This variable is used
*  for gain calibration purpose.
*
*******************************************************************************/
void ADC_SAR_B_SetGain(int16 adcGain)
{
    ADC_SAR_B_countsPer10Volt = (int32)adcGain * 10;
}


/*******************************************************************************
* Function Name: ADC_SAR_B_SetScaledGain
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ADC_SAR_B_SetScaledGain(int32 adcGain)
{
    ADC_SAR_B_countsPer10Volt = adcGain;
}
...
*  ADC_SAR_B_countsPer10Volt variable used.
*
*******************************************************************************/
int16 ADC_SAR_B_CountsTo_mVolts(int16 adcCounts)
{
    ...
    if(ADC_SAR_B_countsPerVolt != 0)
    {   /* Support obsolete method */
        countsPer10Volt = (int32)ADC_SAR_B_countsPerVolt * 10;
    }
    else
    {
        countsPer10Volt = ADC_SAR_B_countsPer10Volt;
    }

    /* Subtract ADC offset */
    adcCounts -= ADC_SAR_B_offset;
    /* Convert to millivolts with rounding */
    mVolts = (int16)( (( (int32)adcCounts * ADC_SAR_B_10MV_COUNTS ) + ( (adcCounts > 0) ?
...
* Global Variables:
*  ADC_SAR_B_offset variable used.
*  ADC_SAR_B_countsPer10Volt used to convert ADC counts to uVolts.
*
*******************************************************************************/
int32 ADC_SAR_B_CountsTo_uVolts(int16 adcCounts)
{
    ...
    */
    uVolts = (( (int64)adcCounts * ADC_SAR_B_10UV_COUNTS ) / countsPer10Volt);
...
* Global Variables:
*  ADC_SAR_B_offset variable used.
*  ADC_SAR_B_countsPer10Volt used to convert ADC counts to Volts.
*
*******************************************************************************/
float32 ADC_SAR_B_CountsTo_Volts(int16 adcCounts)
{
    float32 volts;
    int32 countsPer10Volt;
    ...
    volts = ((float32)adcCounts * ADC_SAR_B_10V_COUNTS) / (float32)countsPer10Volt;
