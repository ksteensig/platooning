...
uint8 Comp_B_initVar = 0u;

/* Internal functions definitoin */
static void Comp_B_trimAdjust(uint8 nibble) ;
...
void Comp_B_Init(void) 
{
    ...
    Comp_B_SetSpeed(Comp_B_DEFAULT_SPEED);

    /* Set default Hysteresis */
    #if ( Comp_B_DEFAULT_HYSTERESIS == 0u )
        Comp_B_CR |= Comp_B_HYST_OFF;
    #else
    ...
    /* Power down override feature is not supported for PSoC5A. */
        ...
        #if ( Comp_B_DEFAULT_PWRDWN_OVRD == 0u )
            Comp_B_CR &= (uint8)(~Comp_B_PWRDWN_OVRD);
        #else 
    ...
    Comp_B_CR |= Comp_B_MX_AO;

    /* Set default sync */
    Comp_B_CLK &= (uint8)(~Comp_B_SYNCCLK_MASK);
    #if ( Comp_B_DEFAULT_BYPASS_SYNC == 0u )
        Comp_B_CLK |= Comp_B_SYNC_CLK_EN;
    #else
        Comp_B_CLK |= Comp_B_BYPASS_SYNC;
    #endif /* Comp_B_DEFAULT_BYPASS_SYNC == 0u */
}
...
{
    Comp_B_PWRMGR |= Comp_B_ACT_PWR_EN;
    Comp_B_STBY_PWRMGR |= Comp_B_STBY_PWR_EN;
...
*
* Global variables:
*  Comp_B_initVar: Is modified when this function is called for the 
...
void Comp_B_Stop(void) 
{
    ...
    Comp_B_PWRMGR &= (uint8)(~Comp_B_ACT_PWR_EN);
    Comp_B_STBY_PWRMGR &= (uint8)(~Comp_B_STBY_PWR_EN);
...
void Comp_B_SetSpeed(uint8 speed) 
{
    ...
    Comp_B_CR = (Comp_B_CR & (uint8)(~Comp_B_PWR_MODE_MASK)) |
                           (speed & Comp_B_PWR_MODE_MASK);

    /* Set trim value for high speed comparator */
    if(speed == Comp_B_HIGHSPEED)
    {
        /* PSoC5A */
        #if (CY_PSOC5A)
        ...
        #if (CY_PSOC3 || CY_PSOC5LP) 
            Comp_B_TR0 = Comp_B_HS_TRIM_TR0;
            Comp_B_TR1 = Comp_B_HS_TRIM_TR1;
        #endif /* CY_PSOC3 || CY_PSOC5LP */
    }
    else
    {
    /* PSoC5A */
        ...
        /* PSoC3, PSoC5LP or later */
        #if (CY_PSOC3 || CY_PSOC5LP) 
            Comp_B_TR0 = Comp_B_LS_TRIM_TR0;
            Comp_B_TR1 = Comp_B_LS_TRIM_TR1;
        #endif /* CY_PSOC3 || CY_PSOC5LP */
    }
...
/*******************************************************************************
* Function Name: Comp_B_GetCompare
********************************************************************************
*
* Summary:
...
{
    return( Comp_B_WRK & Comp_B_CMP_OUT_MASK);
}


/*******************************************************************************
* Function Name: Comp_B_trimAdjust
********************************************************************************
*
* Summary:
...
static void Comp_B_trimAdjust(uint8 nibble) 
{
    ...
    cmpState = Comp_B_WRK & Comp_B_CMP_OUT_MASK;
    ...
    for ( trimCnt = 0u; trimCnt < trimCntMax; trimCnt++ )
	{
        ...
        if ((Comp_B_WRK & Comp_B_CMP_OUT_MASK) != cmpState)
        {
...
uint16 Comp_B_ZeroCal(void) 
{
    ...
    Comp_B_SW0 = 0x00u;
    Comp_B_SW2 = 0x00u;
    Comp_B_SW3 = tmpSW3 & (uint8)(~Comp_B_CMP_SW3_INPCTL_MASK);
    ...
    */
    tmpCR = Comp_B_CR;
    Comp_B_CR |= (Comp_B_CAL_ON | Comp_B_HYST_OFF);
    ...
    /* PSoC3, PSoC5LP or later */
    #if (CY_PSOC3 || CY_PSOC5LP)
        Comp_B_TR0 = Comp_B_DEFAULT_CMP_TRIM;
        Comp_B_TR1 = Comp_B_DEFAULT_CMP_TRIM;
    #endif /* CY_PSOC3 || CY_PSOC5LP */
	
	/* Two phase trim - slow modes, one phase trim - for fast */ 
    if ( (Comp_B_CR & Comp_B_PWR_MODE_MASK) == Comp_B_PWR_MODE_FAST)
    {
        Comp_B_trimAdjust(0u);
    }
    else /* default to trim for fast modes */
    {
        Comp_B_trimAdjust(1u);
		Comp_B_trimAdjust(0u);
    }
    ...
    /* PSoC3, PSoC5LP or later */
    #if (CY_PSOC3 || CY_PSOC5LP)
        return ((uint16)((uint16)Comp_B_TR1 << 8u) | (Comp_B_TR0));        
    #endif /* CY_PSOC3 || CY_PSOC5LP */
}


/*******************************************************************************
* Function Name: Comp_B_LoadTrim
********************************************************************************
*
* Summary:
...
void Comp_B_LoadTrim(uint16 trimVal) 
{
    ...
    /*******************************************************************************
    * Function Name: Comp_B_PwrDwnOverrideEnable
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    ...
    /*******************************************************************************
    * Function Name: Comp_B_PwrDwnOverrideDisable
    ********************************************************************************
    *
    * Summary:
